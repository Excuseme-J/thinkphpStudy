...
class YouMeng {
	...
	function sendAndroidUnicast($devicetoken, $title, $content, $msgId) {
		...
		try {
			...
			$unicast->setPredefinedKeyValue("ticker",           "当湖网格系统新消息");
			...
			$unicast->setExtraField("msg_id", $msgId);
			return $unicast->send();
		} catch (Exception $e) {
			return $e->getMessage();
		}
	...
	function sendIOSUnicast($devicetoken, $title, $content, $msgId) {
		if(strlen($content)>50){
			$content='请点击查看详情';
		}
		try {
			...
			$unicast->setPredefinedKeyValue("device_tokens",    $devicetoken); 
			$unicast->setCustomizedField("msgId", $msgId);
			return $unicast->send();
		} catch (Exception $e) {
			return $e->getMessage();
		}
